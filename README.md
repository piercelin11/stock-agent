...
**觀察清單頁 `/watchlist`**：卡片 gallery（一列 1–4 張，依螢幕寬度）+ 階段分頁（首次突破 / 延續爆發 / 醞釀中，依「連續站上布林上軌天數」即時分類，預設「首次突破」）。每張卡：左上資料新鮮度標示（資料庫有當日資料 ⚡ / 抓盤中即時 🕐）、代號 / 漲跌% / stage pill / 籌碼結論 chip、60 日走勢圖、法人籌碼區塊（突破階段 = 近 5 日淨買超 diverging bar；醞釀階段 = 20 格投信買超日曆 + 投信 / 外資自營兩條全市場百分位進度條 + ⓘ 說明）、底排「量增 / 距年高 / 強度 PR / 突破幅度」（突破階段）或「量增 / K棒 / 力道 / 位階」（醞釀中）。資料源自動切換：資料庫最新交易日 == 今日用資料庫，否則一進頁自動抓 `mis.twse.com.tw` 盤中即時報價。強度 PR 讀最近一次盤後掃描結果。移除鈕。
