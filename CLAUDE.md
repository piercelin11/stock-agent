...
- **`top20-gainers.js`**（專案根目錄，非 TS，未整合進 `/scripts`）：抓當日 TWSE+TPEx 收盤資料，篩一般股票印出漲幅前 20 名。`node top20-gainers.js`。
- **`scripts/backfill-daily-quotes.ts`**：FinMind 逐支回補歷史報價至 `DailyQuote`。回補區間為 `BACKFILL_START_DATE`（預設 `2020-01-01`）到執行當天，實測 FinMind 單支查詢 6 年不會被截斷。`npx tsx scripts/backfill-daily-quotes.ts`（`BACKFILL_LIMIT` 限制測試支數；`BACKFILL_START_DATE=YYYY-MM-DD` 覆蓋起始日）。
- **`scripts/calculate-technical-indicators.ts`**：依 `DailyQuote` 算 MA5/10/20/60、布林通道、量能均線、`volatility20d`/`maxDrawdown20d`/`atr20`/`rsi14`/`macdStatus`，寫入 `TechnicalIndicator`（對每支股票的全部歷史 `DailyQuote` 逐日重算）。匯出 `calculateTechnicalIndicators(codes?: string[])`——傳代號陣列只重算那幾支（用於某支事後補齊報價後單獨補指標），不傳跑全市場一般股票。`npx tsx scripts/calculate-technical-indicators.ts`（全市場）或 `npx tsx scripts/calculate-technical-indicators.ts 4104 2330`（指定股票）。
- **`scripts/run-screener.ts`**：讀 `scripts/screener-conditions.json` 條件，對最新交易日跑全市場布林篩選，寫入 `data/screener-results/{date}.json`。匯出 `runScreener()`。支援 `bollinger_breakout`/`volume_surge`/`volume_min`（單日條件）與 `bandwidth_squeeze`（連續 N 天條件）。`npx tsx scripts/run-screener.ts`。
- **`scripts/fill-daily-quotes.ts`**：補齊全市場報價，自動新增 `Stock`、upsert `DailyQuote`。匯出 `fillOneDayTwse(date)`（`MI_INDEX`，可補任意歷史日）與 `fillTodayTpex(expectedIsoDate?)`（TPEx OpenAPI，只能拿「目前最新一天」，日期不符會跳過寫入並回傳 `isStaleDate: true`）。對外 fetch 走 `http.ts` 的 `fetchJson`（3 次 retry + 30s timeout）。`npx tsx scripts/fill-daily-quotes.ts --date=YYYY-MM-DD`。
- **`scripts/calculate-industry-heat.ts`**：依 `DailyQuote` 算各產業每日等權熱度寫入 `IndustryHeatSnapshot`（純資料庫計算，零 API）。匯出 `calculateOneDayHeat(date)`。`npx tsx scripts/calculate-industry-heat.ts`（最近交易日）或 `--backfill 20`。
- **`scripts/fill-gap-valuation.ts`**：抓指定日期 TWSE（`BWIBBU_d`）+ TPEx（`peQryDate`）估值寫入 `StockValuation`，兩邊皆可查任意歷史日期。對外 fetch 走 `http.ts` 的 `fetchJson`（3 次 retry + 30s timeout）。匯出 `fillOneDayValuation(date)`。`npx tsx scripts/fill-gap-valuation.ts --date=YYYY-MM-DD`（不帶參數抓今天）。
- **`scripts/update-shares-outstanding.ts`**：下載 MOPS 股本 CSV 更新 `Stock.sharesOutstanding`。**獨立手動執行，月頻，不進 daily pipeline**。`npx tsx scripts/update-shares-outstanding.ts`。
- **`scripts/fill-institutional-trading.ts`**：抓指定日期 TWSE（`T86`，可查任意歷史日）+ TPEx（`tpex_3insti_daily_trading`，只能拿最新一天）三大法人買賣超寫入 `InstitutionalTrading`。只 upsert 已存在的一般股票，不新增股票。對外 fetch 走 `http.ts` 的 `fetchJson`（3 次 retry + 30s timeout）。匯出 `fillOneDayInstitutional(date)`。`npx tsx scripts/fill-institutional-trading.ts --date=YYYY-MM-DD`。**尚無下游消費者**（`calculate-screen-score.ts`/`calculate-breakout-strength.ts` 都不讀）。
- **`scripts/daily-pipeline.ts`**：每日排程主控腳本，五步：補今日 TWSE+TPEx 報價 → 抓今日籌碼 → 抓今日估值 → 算技術指標 → 算產業熱度。任一步驟失敗印錯誤並非 0 結束。**非當日資料防呆**：報價步驟後若 TWSE+TPEx 皆無當日資料，提前結束整支 pipeline 並印警告數。`npx tsx scripts/daily-pipeline.ts`。搭配 `daily_pipeline.plist`（每天 17:00 觸發，**尚未部署到 launchd**）。
- **`scripts/http.ts`**：`daily-pipeline.ts` 三個抓取步驟（`fill-daily-quotes` / `fill-institutional-trading` / `fill-gap-valuation`）共用的 HTTP 工具，純函式庫（無 Prisma/CLI）。匯出 `fetchJson<T>(url, options?)`：每次嘗試帶 `AbortSignal.timeout`（預設 30s），對網路層拋錯（`ECONNRESET`/`terminated`/timeout）與 5xx/429 自動退避重試（預設 3 次，2s→4s），對其他 4xx 不重試。存在理由：pipeline 無人值守執行，裸 fetch 遇一次 TLS 連線中斷就整條掛掉（2026-08-28 事故）。**回應內容判斷（非交易日 `stat`/`totalCount`/空陣列）仍留在各呼叫端**，`fetchJson` 只負責「拿到 2xx 並 `res.json()`」。`backfill-*.ts` 等手動腳本目前未接（有人在旁邊看、單支失敗不中斷），要用可自行接。
- **`scripts/fetch-candidate-details.ts`**：讀 `data/screener-results/{日期}.json` 候選股清單，逐支抓籌碼（`InstitutionalTrading`）、月營收（`MonthRevenue`）、季報（`FinancialStatement`）、新聞（`NewsArticle`/`NewsStock`，依 `link` 去重）寫入資料庫。單步失敗不中斷，結束印失敗總結。`npx tsx scripts/fetch-candidate-details.ts --date=YYYY-MM-DD`。
- **`scripts/calculate-screen-score.ts`**：對最新交易日全市場一般股票，用九因子加權（value/size/liquidity/momentum/reversal/activity/stability/theme_heat/topic_alignment）算 `screenScore` 排名，是全市場排序（非布林篩選）。結果不寫資料庫，輸出至 `data/screen-score-results/{date}.json`。匯出 `calculateScreenScore(date)`。`npx tsx scripts/calculate-screen-score.ts --date=YYYY-MM-DD`。依賴 `TechnicalIndicator`/`StockValuation`/`IndustryHeatSnapshot`/`sharesOutstanding` 為當天最新資料。
- **`scripts/calculate-breakout-strength.ts`**：篩出帶量帶價第一根突破布林的股票並依訊號強度排名（三層：觸發→資格門檻→強度評分，評分公式皆單調遞增，不用鐘型曲線）。共用邏輯（`GATES`/`WEIGHTS`/評分函式）在 `breakout-shared.ts`。結果不寫資料庫，輸出至 `data/breakout-strength-results/{date}.json`。匯出 `calculateBreakoutStrength(date)`。`npx tsx scripts/calculate-breakout-strength.ts --date=YYYY-MM-DD`。
- **`scripts/breakout-shared.ts`**：`calculate-breakout-strength.ts` 與 `check-intraday-breakout.ts` 共用的純函式庫（無 Prisma/CLI）。內容：`GATES`/`WEIGHTS`/`TRIGGER_VOLUME_RATIO` 等常數、`rankScore`/`clip`/`fetchHistoryWindow`，以及 `computeVolumeStrength`/`computeBreakoutMargin`/`computeFirstBar`/`computeBase`/`computeProximityToHigh`/`computeMarketWideReturns`/`computeCandleShape` 七項評分函式。目前 `computeBreakoutMargin` 為鐘型曲線（乖離 >3% 遞減）、`GATES.minMarketCap` 為 30 億。
- **`scripts/check-intraday-breakout.ts`**：盤中一次性快照篩選，把 `calculate-breakout-strength.ts` 的三層邏輯套用在 `mis.twse.com.tw` 即時報價上（社群逆向工程端點，非官方文件）。**一次性手動執行，不排程、不接進 daily-pipeline**，不支援 `--date`。批次查詢（120 檔/批，1.5 秒節流），`elapsedRatio` 現算預估全天量。結果輸出至 `data/intraday-breakout-snapshots/{timestamp}.json`。`npx tsx scripts/check-intraday-breakout.ts`。
- **`scripts/backfill-institutional-trading.ts`**：逐支一般股票用 FinMind 回補歷史三大法人買賣超至 `InstitutionalTrading`（TWSE 可回補任意歷史，TPEx 受端點限制效果有限）。回補區間為 `BACKFILL_START_DATE`（預設 `2020-01-01`）到執行當天，實測 FinMind 單支查詢 6 年（一天 5 個法人分類、共約 7400 筆）不會被截斷。單支失敗不中斷。`npx tsx scripts/backfill-institutional-trading.ts`（`BACKFILL_LIMIT=10` 小量測試；`BACKFILL_START_DATE=YYYY-MM-DD` 覆蓋起始日）。**已執行完成（首輪）**：`InstitutionalTrading` 現有 2025-05-02 起 324 個交易日、約 59 萬筆、1989 檔（2026-08-27 起以 `2020-01-01` 起始日重跑回補至 6 年）。
- **`scripts/backfill-6y.sh`**：一次跑完 6 年歷史回補的四步接力：`backfill-daily-quotes.ts` → `backfill-institutional-trading.ts` → `calculate-technical-indicators.ts`（天生就重算整段）→ `calculate-industry-heat.ts --backfill 1600`（涵蓋 6 年交易日）。`set -e` 任一步失敗即停。log 寫到 `logs/backfill_6y_{timestamp}.log`。用法：`nohup sh scripts/backfill-6y.sh > /dev/null 2>&1 &`，`tail -f logs/backfill_6y_*.log` 看進度。休眠／闔蓋只會暫停 process，開蓋後從暫停處接著跑。
- **`scripts/calculate-accumulation-score.ts`**：盤後選股 v2「投信吃貨訊號」——找還沒出現第一根突破、但籌碼/技術面在醞釀的股票，與 `calculate-breakout-strength.ts`（找已發生的突破）互補。計分為**乘法結構**：`最終分數 = 籌碼分數 × 技術就緒係數`。籌碼分數（主排序依據）= 投信分數 × 0.7 + 其他法人分數 × 0.3；技術就緒係數 = 壓縮度/窒息量合成後線性映射到 `READINESS_FLOOR`~1.0（下限 0.5，技術面沒收斂只打折不歸零）。候選池先剔除「今日已站上布林上軌」（與突破清單互斥）與「近 20 日均量 < `MIN_AVG_VOLUME_SHARES`(=500 張)」（低流動性死股）。結果不寫資料庫，輸出至 `data/accumulation-score-results/{date}.json`（`code`+`date` 欄位格式對齊突破腳本，方便日後回測命中率）。匯出 `calculateAccumulationScore(date)`。`npx tsx scripts/calculate-accumulation-score.ts --date=YYYY-MM-DD`。**獨立手動執行，不進 daily-pipeline**。所有待校準參數（`CHIP_WEIGHTS`/`TRUST_SUB_WEIGHTS`/`TECH_WEIGHTS`/`READINESS_FLOOR` 等）集中在 `accumulation-shared.ts`，首版未校準（前段偏大型股）。
- **`scripts/accumulation-shared.ts`**：`calculate-accumulation-score.ts` 的純函式庫（無 Prisma/CLI）。內容：視窗/門檻常數（`INSTITUTIONAL_WINDOW_DAYS=20`、`SQUEEZE_VOLUME_WINDOW_DAYS=5`、`MIN_AVG_VOLUME_SHARES`、`READINESS_FLOOR` 等）、待校準權重常數、`rankScore`/`clip`，以及 `computeTrustRawMetrics`/`computeOtherInstitutionRatio`/`computeQuietVolumeRatio`/`combineChipScore`/`combineTrustScore`/`computeReadinessCoefficient`/`combineFinalScore`。壓縮度分數直接沿用 `breakout-shared.ts` 的 `computeBase`（不重造）。
