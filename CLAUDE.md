...
- **Server Actions**（不建 REST/GraphQL）：檔案放 `lib/actions/*.ts`，檔頭 `"use server"`。action 只做「呼叫 Prisma / 純函式 → 回傳可序列化 plain object」。回傳 `Date` 要先轉字串、`Decimal` 要 `.toNumber()` / `.toString()`、`BigInt`（`volume` / `*NetBuy`）要 `Number(...)`，在 action 邊界轉掉（`Decimal` 直接丟 client component 會壞）。現有：`health.ts`（`getDbHealth`）、`screening.ts`（`runScreening`）、`watchlist.ts`（`listWatchlist` / `addToWatchlist` / `removeFromWatchlist` / `updateWatchlistItem`，mutation 後 `revalidatePath("/watchlist")`）、`intraday.ts`（`startIntradayScan` / `getIntradayProgress` / `getIntradayResult`——盤中掃描背景任務，spawn 子進程 + 輪詢 `progress.json`）。
