...
- **`/screening` 頁**：Server Component 外殼改 `async`，頁面標題下、`ScreeningPanel` 之上放 `<RegimeBanner variant="strip" />`（不擋按鈕、不改 `runScreening`）。兩個策略分頁（「第一根突破」= `calculateBreakoutStrength`、「冷水區醞釀」= `calculateAccumulationScore`）。按鈕 → `runScreening` **同步在 Next.js 進程內 import 呼叫純函式跑「最新交易日」**（`DailyQuote` 裡 `securityType="stock"` 的 `max(date)`），不 spawn 子進程（單日約 1900 檔、秒級；背景任務模式是給幾百日回測用的）。傳前端 Prisma 單例給純函式（`{ prisma }`），純函式偵測 `options.prisma !== undefined` 時不 `$disconnect`。結果**不進 DB**（沿用純函式既有 `data/*-results/{date}.json` 落地）。表格可排序、點列展開評分明細、勾選 → `addToWatchlist({ codes, source: strategy })`。
  - **第三個 tab「盤中即時掃描」走背景任務模式**（不是同步跑）：`ScreeningPanel` 內部 tab 值擴成 `"breakout" | "accumulation" | "intraday"`（local 型別，不動 `runScreening` 的 `ScreeningStrategy`）。「開始掃描」→ `lib/actions/intraday.ts` 的 `startIntradayScan()` spawn detached 子進程跑 `_run-intraday-scan.ts` → 子進程逐批寫 `data/intraday-breakout-snapshots/progress.json` → client `setInterval(2s)` 輪詢 `getIntradayProgress()` 顯示進度條 → `status: "done"` 時 `getIntradayResult()` 讀最新 `{timestamp}.json` 轉 rows。同時只允許一個掃描（`progress.json` 單檔，`updatedAt` 超過 90 秒視為死掉允許重跑）。切走只停 client 輪詢、不中斷子進程；切回來若還 `running` 重新輪詢。跑完**重用同一套表格 / 排序 / 展開明細（7 分項與 breakout 同形狀）/ 勾選加入**，`source: "intraday"`。非盤中時段只顯示警語不阻擋。`intraday.ts` 只 `import type { CandidateResult }`，不 import `checkIntradayBreakout` 本體（避免把 `dotenv/config` / `node:fs` 拉進 bundler）。
...
- **背景任務**：模式是「Server Action `spawn` 一個 detached 子進程 → 子進程每步覆寫 `data/xxx/progress.json` → 另一個 Server Action 輪詢讀檔 → client component `setInterval` poll」。**`spawn` 用 `process.execPath` + `node_modules/tsx/dist/cli.mjs` 絕對路徑 + 目標腳本當參數**，不要用 `pnpm tsx` / `npx tsx`（detached 子進程解析 launcher 慢、PATH 依賴）。`.npmrc` 設 `node-linker=hoisted`，所以 `node_modules/tsx/dist/cli.mjs` 是扁平路徑、解析得到。原子寫 `progress.json`（先寫 `.tmp` 再 `renameSync`）避免輪詢讀到寫一半。**已有正式使用者**：`lib/actions/intraday.ts`（`/screening` 盤中即時掃描 tab）+ `scripts/screening/_run-intraday-scan.ts` runner。（PoC 檔案 `scripts/_poc/` + `lib/actions/poc.ts` + `components/PocRunner.tsx` 已於 ROADMAP 3.1 刪除。）
...
- **`daily-pipeline.ts`**：每日排程主控腳本，六步：補今日 TWSE+TPEx 報價 → 抓今日籌碼 → 抓今日估值 → 算技術指標 → **算大盤濾網（市場狀態燈號）** → 算產業熱度。前四步任一失敗印錯誤並非 0 結束；第 5 步（大盤濾網）為**非關鍵路徑**，失敗只印警告 + `warningCount++`，不中斷。**非當日資料防呆**：報價步驟後若 TWSE+TPEx 皆無當日資料，提前結束整支 pipeline 並印警告數。無 `isMain` guard，`import` 這支即會執行 `main()`。`pnpm tsx scripts/pipeline/daily-pipeline.ts`。搭配 `daily_pipeline.plist`（每天 17:00 觸發，**尚未部署到 launchd、檔案也尚未建立**）。
...
- **`calculate-market-regime.ts`**：大盤濾網（市場狀態燈號）pipeline 薄殼。`calculateOneDayRegime(date, prisma)` → 呼叫 `scripts/lib/market-regime.ts` 的 `calculateMarketRegime` → 原子寫 `data/market-regime/{date}.json`（**不進 DB**：整個市場每天一標籤、資訊量小、盤中多次取允許一天多筆覆寫、不做回測 → 建表不划算）。`daily-pipeline.ts` 第 5 步（非關鍵路徑），CLI `pnpm tsx scripts/pipeline/calculate-market-regime.ts`（無參數跑 DB 最新交易日）／`--date=YYYY-MM-DD`（補算）。**目前為 Step 1**：只有「市場寬度 breadth」單維度上線；Step 2（TAIEX 落地 + 指數位置 / MA60 斜率）待做。
...
- **`market-regime.ts`**：大盤濾網（市場狀態燈號）純函式庫（無 CLI，允許查 DB，比照 `breakout-shared.ts`）。`calculateMarketRegime(date, { prisma, config? })` → `MarketRegimeResult`（`label: "bullish"|"neutral"|"bearish"` / `totalScore` / `dimensions.{indexPosition,ma60Slope,breadth}` 各為 `DimensionScore | null` / `stage`）。三維度各投 `-1|0|1`：**breadth**（Step 1 就有）= 該日全市場 `securityType="stock"` 有 `DailyQuote` 的股票中 `close > 當日 TechnicalIndicator.ma60` 佔比，`>55%` +1 / `<45%` -1 / 分母 <500 → `degraded`；**indexPosition** / **ma60Slope**（Step 2，TAIEX 落地後）= TAIEX 收盤 vs 自己 MA60（連 N 日緩衝）／MA60 近 5 日斜率。`stage` 由 `hasTaiexIndicatorForDate()` 自動判定（有 → `step2-full` 三票合成、沒有 → `step1-breadth-only` 只看 `breadth.score`，優雅降級）。門檻集中在 `DEFAULT_REGIME_CONFIG`（首版未校準）。查 DB helper 同檔：`fetchBreadthInputs` / `fetchTaiexCloseVsMa60` / `fetchTaiexMa60Series` / `hasTaiexIndicatorForDate`。**目前只有 Step 1（breadth）上線。**
