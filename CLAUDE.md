...
- **`/screening` 頁（ROADMAP 4.5.3：單頁 + 階段 filter，三分頁已收斂）**：Server Component 外殼 `async`，頁面標題下、`ScreeningPanel` 之上放 `<RegimeBanner variant="strip" />`。`ScreeningPanel` 一顆「跑掃描」按鈕，`getScanMode()`（`lib/actions/signal-scan.ts`）決定文案 / 行為：
  ...
  - **表格欄位**（UI 改版後）：排名 / 代號 / 名稱 / **價格**（`estimated` 尾隨琥珀「估」badge）/ 漲跌%（百分比、漲紅跌綠）/ **狀態**（階段中文 pill，`breakout-day`=`bg-destructive/10 text-destructive`、`extended`=`bg-warning/10 text-warning`、`pre-breakout`=`bg-muted text-muted-foreground`）/ **量增**（`x{volumeRatio}`，醞釀中 `—`）/ **籌碼**（一句話結論 chip，醞釀中 `—`，盤中沿用昨日籌碼附「昨」badge）/ 總分。`Column` 有 `sortable?: boolean`（`inst` 欄 `false`，不可排序）。舊「追」badge 從價格欄移到籌碼 chip（`margin-chasing` → 紅「融資追價」）。舊獨立「降級項目」欄移到展開列底部。
  - **展開列三欄**（`components/screening/SignalDetail.tsx`，原 `Detail` function 搬出）：`pre-breakout` → **只有左側線圖**（不渲染法人 / 突破因子 / 舊評分明細網格）。`breakout-day` / `extended` → 三欄卡（`grid-cols-1 lg:grid-cols-3`，深色卡）：左 `<SignalSparkPanel code>`（client，按需 `getSignalSpark(code)` 撈近 60 交易日走勢）/ 中 `<InstitutionalFlow inst warnings>`（投信 / 外資 diverging bar，**柱長用 `inst.trustRatio` / `inst.foreignRatio`（近 5 日淨買超 ÷ volumeMa20），不是股數不是分數**；箭頭 ▲/▼ = 今日單日方向；翻臉柱子 `opacity-50`）/ 右 `<BreakoutFactorBars factors rs>`（4 列，**長條用因子內部分數 0~100，不用原始值**；分級 `>=70` 綠 / `40~69` 琥珀 / `<40` 紅；不放 base 因子）。詳細規格與設計理由見 `docs/PROGRESS.md`「Screening 頁表格 UI 改版」段。
  - `priceSource === "estimated"`（realtime 缺成交價、`h` 代入）列尾「估」badge（`bg-warning/10 text-warning`）+ row 淡色 + 展開多一行提示。
  - `SignalResult.warnings` 含 `"margin-chasing"` → 展開最上方紅框提示 + 籌碼 chip 轉紅。**`warnings` 與 `degraded` 語意分開**：`degraded` = 因資料不足所以不確定；`warnings` = 資料充足、系統明確發現的風險訊號。只有 `breakout-day`/`extended` 的 `SignalResult` 會有非空 `warnings`（pre-breakout 恆 `[]`）。與 `SignalScanOutput.warnings`（管線層級警語）是不同層級。
  - **`SignalResult` 的 UI 專用欄位**（UI 改版新增，皆 optional，只在 `breakout-day` / `extended` 有值）：`volumeRatio?`（觸發量比）、`inst?: { trustRatio, foreignRatio, todayTrustDir, todayForeignDir }`（法人籌碼中繼值）、`factors?: { breakoutMarginScore, breakoutMarginPct, proximityShortScore, proximityLongScore, proximityShortPct, proximityLongPct }`（突破因子分數 + 原始值）。純回傳值擴充，**不動評分**（`computeInstitutionalFlow` / `computeProximityToHigh` 多回中繼值，score / degraded / marginChasing 邏輯不變）。
...
- **`run-signal-scan.ts`（ROADMAP 4.5.3 統一評分管線，唯一的選股腳本）**：跑全市場一般股票 → 單一 gate（`minMarketCap` 30 億 + 當日 `volume` ≥ 1M 股 + `volumeMa20` ≥ 500k 股）→ 依「連續站上布林上軌天數」打階段標籤（`pre-breakout` / `breakout-day` / `extended`）→ 同一份因子分、階段套**不同合併方式**（`pre-breakout` = 乘法「籌碼分 × 就緒係數」，下限 0.5；`breakout-day`·`extended` = 8 分項加權和）→ **各階段各自排名** → 落地 `data/signal-scan-results/{date}.json`（eod）/ `{timestamp}.json`（realtime）+ `params`。**資料源自動切換**：查「今天（Asia/Taipei）有 `DailyQuote` 嗎」→ 有 `source: "eod"`（讀 DB）/ 沒有 `source: "realtime"`（打 `mis.twse.com.tw`，缺成交價用當日最高價 `h` 代入、`candleShape` 走 degraded、`priceSource: "estimated"`；`z`/`h` 皆缺跳過）。「觸發量比 ≥ 2」只對 breakout 階段當進榜門檻。匯出 `runSignalScan(date, { prisma?, config?, source?, now? })`（傳 `prisma` 不 `$disconnect`；`config` 為 `DeepPartial<SignalScanConfig>`）。CLI：`pnpm tsx scripts/screening/run-signal-scan.ts`（自動判斷）／`--date=YYYY-MM-DD`（強制 eod）／`--source=realtime|eod`。**獨立手動執行，不進 daily-pipeline**。**三處偏離舊 breakout 預設**：`baseCurve` 深度/時長 = 0.4/0.6（舊 0.6/0.4）、`breakoutMargin` 改單調遞增（> 3% 封頂 100 不倒扣）、7 分項各 ×0.90 勻出 `institutionalFlow: 0.10`。
...
- **`signal-factors/`（ROADMAP 4.5.3 統一因子庫，4.5.4 目錄化）**：`run-signal-scan.ts` 與 `lib/actions/dashboard.ts` 的單一 import 點（都 import `signal-factors/index`）。4.5.4（2026-09-01）把原「`signal-factors.ts` 薄 re-export 層 + `breakout-shared.ts` / `accumulation-shared.ts` 本體」整併成這個目錄，舊三支同批退役移除（見 `scripts/screening/` 段）。**子檔分工**：
  ...
  - `institutional.ts` — `computeInstitutionalFlow()`（三大法人流向：近 5 日投信+外資淨買超各 ÷ `volumeMa20` → clip `[0,0.5]` 線性 0~100 → 投信 0.6/外資 0.4；突破當日淨賣超 → `min(score, 40)`；當日法人 null → 不封頂但 degraded。**另收 `marginSurgePercentile` 輸入、回 `marginChasing` 布林**——`marginSurgePercentile !== null && > surgePercentileThreshold(80) && 突破當日法人淨賣超` → true。**不動 score**，只給呼叫端掛 `SignalResult.warnings`）、`computeMarginSurgePercentile()`（純函式：融資餘額近 `lookbackDays`(5) 日累積變化率，對「這檔過去 `historyWindowDays`(40) 天各自的同種變化率」母體取百分位，**用嚴格小於**避免平盤誤觸發；母體有效樣本 < `minHistoryDays`(20) → null）。
  ...
  - `config.ts` — `SignalScanConfig` / `DEFAULT_SIGNAL_CONFIG` / `resolveSignalConfig`（三處偏離舊 breakout：`baseCurve` 0.4/0.6、`breakoutMargin` monotone、7 項各 ×0.90 勻出 `institutionalFlow: 0.10`；`breakout.institutionalFlow.marginChasing` 子區塊 = `margin-chasing` 觸發參數，首版拍腦袋待校準）、`baseCurveFor()`。
  - 單測 `signal-factors/factors.test.ts`（`node:test` + `pnpm tsx --test scripts/lib/signal-factors/factors.test.ts`，31 案）。
- **`mis-quotes.ts`（MIS 即時報價抓取，純 Node 函式庫）**：2026-08-31 從當時的 `check-intraday-breakout.ts` 抽出（該檔已於 4.5.4 退役移除），現由 `run-signal-scan.ts` 的 realtime 路徑使用。`fetchMisBatch` / `fetchAllMisQuotes(stocks, onBatch?)` / `parseMisDate` / `computeElapsedRatio` / `MisQuote` 型別 / `BATCH_SIZE`(120) / `BATCH_DELAY_MS`(1500)。缺成交價（`z` 為 `-`/缺）的 row **不跳過**、`price: null` 保留，由呼叫端決定代入（`run-signal-scan.ts` 用 `h`）。
